% Espacement variable entre les diaclases
clear, close, clc, tic
% vik = open('vik.mat'); vik = vik.vik;
...
nx          = 351;              ny          = nx;                   % Numerical resolution
...
err_diaclase1	= 0.001;					% error of diaclase
a_diaclase1		= 25;	a_diaclase2 = 135;	% angle of both diaclase set
esp_dia			= [0.01 0.025 0.35];		% spacing of diaclase
TXT				= esp_dia*20;				% text for title
Pt				= zeros(1,size(esp_dia,2));	% set the control point vector

for i = 1:size(esp_dia,2)
	% Initialization
	...
	% first diaclase
	d_dia = esp_dia(i)/sind(a_diaclase1);
	diaclase1 = (-0.75 : d_dia : 0.75);
	for j = 1:size(diaclase1,2)
		ETA((Y < X*tand(a_diaclase1)+err_diaclase1 + diaclase1(j)) & (Y > X*tand(a_diaclase1)-err_diaclase1 + diaclase1(j)))  = etaTunnel;
	end
	% second diaclase
	d_dia = esp_dia(i)/sind(a_diaclase2);
	diaclase2 = (-0.75 : d_dia : 0.75);
	for j = 1:size(diaclase2,2)
		ETA((Y < X*tand(a_diaclase2)+err_diaclase1 + diaclase2(j)) & (Y > X*tand(a_diaclase2)-err_diaclase1 + diaclase2(j)))  = etaTunnel;
	end
	
	[inside, beton, txt] = tunnel(4,0.214,X,Y,0,0,"oui",0.014);
	...
	while err_absolute(end)>tol||err_relative(end)>tol; iter = iter+1;  % START of iteration loop
		...
		if mod(iter,nout)
			...
			disp(['fig : ',num2str(i),'/3'])
		end	
	
		Pt(i) = TII(176,257)/(2*eta_B*abs(D_B));
	
	end			% END of iteration loop ---------------------------------------
	figure(1), z = figure(1);
	subplot(1,3,i), hold on
	title(['Espacement = ',num2str(TXT(i)), 'm'], Position=[0 0.59])
	pcolor(X,Y,TII/(2*eta_B*abs(D_B))), shading interp, axis equal tight
	c=colorbar; colormap('jet'),
	if i == 3, ylabel(c,'2e contrainte invariante, T_{II} / (2\eta_BD_B) [ ]'), end
	if i == 1, ylabel('Hauteur [ ]'), end
	xlabel('Largeur [ ]'), fontsize(20,"points"), axis equal tight 
end

saveas(z,fullfile('C:\Users\schin\OneDrive\Documents\Travail de Bachelor\Figure','Fig16_Espacement_diaclase.tif'));
