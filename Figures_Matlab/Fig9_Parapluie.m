...
[inside, beton, txt] = tunnel(13,sze,X,Y,0,0,"oui",0.014);		% shape of the tunnel (function)
if beton == 0; ETA(:,:) = eta_B; else ETA(beton==1) = etaBeton; end	% enveloppe de béton
...
while err_absolute(end)>tol||err_relative(end)>tol; iter = iter+1;  % START of iteration loop
	...
	if mod(iter,nout)==0 % Visualization during calculation
		...
		title(txt), ylabel(c, '2e contrainte invariante, T_{II} / (2\eta_BD_B) [ ]'), 
		xlabel('Largeur [ ]'), ylabel('Hauteur [ ]')
		colormap('jet'), fontsize(28,"points"), drawnow
    end
...
saveas(z,fullfile('C:\Users\schin\OneDrive\Documents\Travail de Bachelor\Figure','Fig9_Parapluie.tif'));
