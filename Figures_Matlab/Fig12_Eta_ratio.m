% Changement du ratio de viscosité
clear, close, clc, tic
etaRatio    = logspace(-1,7,60);		% Viscosity ratio (matrix/inclusion)
nout        = 2e3;		% Parameter for the visualisation 
iter_max	= 5e4;		% Parameter to break
...
a			= 0.1;		% taille de la galerie

Pt_1 = zeros(1,size(Ar,2));
Pt_2 = zeros(1,size(Ar,2));

for i = 1:size(etaRatio,2)
	% Initialization
	P           = zeros(nx,  ny);		% Pressure
	ETA         = eta_B*ones(nx, ny);	% Viscosity
	RHO_G       = rhog*ones(nx, ny);	% Gravity
	% Define Geometric situation
	[inside, beton, txt] = tunnel(1,a,X,Y,0,0,"non",0.014);		% shape of the tunnel (function)
	inside          = (X.^2 + Y.^2) < a^2;
	ETA(inside)     = eta_B/etaRatio(i);  
	RHO_G(inside)   = rhog/1e3;  
	...
	Pt_1(i) = TII(X==0 & Y==0.12)/(2*eta_B*abs(D_B));
	Pt_2(i) = TII(X==0.12 & Y==0)/(2*eta_B*abs(D_B));

	clc, disp(['loading : ',num2str(i/size(etaRatio,2)*100) '%'])

end			% END of iteration loop ---------------------------------------

figure(1), z = figure(1);
semilogx(etaRatio(1:i),Pt_1, etaRatio(1:i),Pt_2, LineWidth=2), grid on 
xlabel('Rapport de viscosité'), ylabel('2e contrainte invariante, T_{II} / (2\eta_BD_B), [ ]')
xlim([1e-1 1e7]), legend('Plafond','Mur de droite'), fontsize(20,"points")

saveas(z,fullfile('C:\Users\schin\OneDrive\Documents\Travail de Bachelor\Figure','Fig12_Eta_ratio.tif'));
