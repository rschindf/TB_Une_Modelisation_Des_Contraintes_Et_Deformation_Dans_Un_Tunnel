...
nx          = 751;              ny          = nx;                   % Numerical resolution
...
a_interf	= 15;					% angle of the interface
% Initialization
P           = zeros(nx,  ny);		% Pressure
ETA         = eta_B*ones(nx, ny);	% Viscosity
RHO_G       = rhog*ones(nx, ny);	% Gravity
% Define Geometric situation
ETA((X + Y*tand(90-a_interf)) > 0) = etaVersch;			% compartiment de haut-droite
[inside, beton, txt] = tunnel(4,0.214,X,Y,0,0,"oui",0.014);

if beton == 0; ETA(:,:) = eta_B; else ETA(beton) = etaBeton;	end
ETA(inside) = etaTunnel;
...
figure(1)
subplot(1,2,2)
pcolor(X,Y,TII/(2*eta_B*abs(D_B))),shading interp,axis equal tight,c = colorbar;
xlabel('Largeur [ ]'), ylabel(c, '2e contrainte invariante, T_{II} / (2\eta_BD_B), [ ]')
fontsize(20,"points"), title('2 - Contraintes'), hold off

saveas(z,fullfile('C:\Users\schin\OneDrive\Documents\Travail de Bachelor\Figure','Fig13_Interface_initiale.tif'));
