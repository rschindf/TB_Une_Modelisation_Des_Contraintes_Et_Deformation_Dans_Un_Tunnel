...
clear, close, clc, tic
% Argand number
Ar = logspace(-3,2,50); Ar = [Ar(1) Ar(10:end)];
Pt_1 = zeros(1,size(Ar,2));		% Vector for the first point
Pt_2 = zeros(1,size(Ar,2));		% Vector for the second point
for i = 1:size(Ar,2)
clearvars -except Ar i Pt_1 Pt_2
	...
	nx          = 201;              ny          = nx;                   % Numerical resolution
	...
	ETA(inside)     = eta_B/etaRatio;	% set viscosity value
	...
	Pt_1(i) = TII(X==0 & Y==0.175)/(2*eta_B*abs(D_B));
	Pt_2(i) = TII(X==0.175 & Y==0)/(2*eta_B*abs(D_B));
	clc, disp(['loading : ',num2str(i/size(Ar,2)*100), '%'])

end

saveas(z,fullfile('C:\Users\schin\OneDrive\Documents\Travail de Bachelor\Figure','Fig5_Argand_pt.tif'));

toc 
% visualisation : Changement des contraintes avec le nombre d'Argand
figure(1); z = figure(1); clf
subplot(1,2,2)
semilogx(Ar,Pt_1, Ar,Pt_2, LineWidth=2), grid on, ax=gca; ax.YAxisLocation = "right";
xlabel('Nombre d''Argand'), ylabel('2e contrainte invariante, T_{II} / (2\eta_BD_B) [ ]'), fontsize(20,"points")
title('Chemin de contrainte des deux points'), xline(1), xlim([1e-3 1e2])
legend('Pt. A','Pt. B','', Location='northwest'),

subplot(1,2,1)
pcolor(X,Y,TII/(2*eta_B*abs(D_B))), colormap('jet'), c = colorbar; shading interp, axis equal tight
xlabel('Largeur [ ]'), ylabel('Hauteur [ ]'), title('Position des points de contrôle', Position=[0 0.57]), fontsize(20,"points")
ylabel(c,'2e contrainte invariante, T_{II} / (2\eta_BD_B) [ ]', FontSize=16)
hold on, scatter([0 0.175],[0.175 0],100,'w','*'), 
txt = ['A'; 'B']; xtxt = [0.025 0.2]; ytxt = [0.175 0]; text(xtxt,ytxt,txt,FontSize=16,Color='w')
hold off

saveas(z,fullfile('C:\Users\schin\OneDrive\Documents\Travail de Bachelor\Figure','Argand_Pts_final_v2.tif'));
