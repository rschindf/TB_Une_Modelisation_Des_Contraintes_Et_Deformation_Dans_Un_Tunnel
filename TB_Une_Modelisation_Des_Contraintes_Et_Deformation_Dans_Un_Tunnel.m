...
% Modified by Robin Schindfessel for his Bachelor Thesis (2024). https://github.com/rschindf/TB_Une_Modelisation_Des_Contraintes_Et_Deformation_Dans_Un_Tunnel 
clear, close, clc, tic
...
[inside, beton, txt] = tunnel(1,sze,X,Y,0,0,"oui",0.014);		% shape of the tunnel (function)
...
while err_absolute(end)>tol||err_relative(end)>tol; iter = iter+1;  % START of iteration loop
    ...
    if mod(iter,nout)==0	% Visualization during calculation
        subplot(221),pcolor(X,Y,(P-P(1,end))/(2*eta_B*abs(D_B))),shading interp,axis equal,axis tight,
		colorbar, title({'A) Pression, P / (2\eta_BD_B)'}),ylabel('Hauteur [ ]')
        subplot(222),pcolor(X,Y,log10(ETA/eta_B)), shading interp, hold on
        title(['B) Log_{10} de la viscosité, \eta / \eta_B, et flèches de vitesse'])
        ...
        title(['C) 2e contrainte invariante, T_{II} / (2\eta_BD_B)']), xlabel('Largeur [ ]'), ylabel('Hauteur [ ]')
        subplot(224),loglog(1:length(err_absolute),err_absolute,1:length(err_relative),err_relative),
		title('D) Convergence de l''erreur (résiduelle)'),xlabel('Itérations'),ylabel('Résidu')
        axis([1 iter+1e4 tol/10 1e6]), hold on,plot([1 iter+1e4],[tol tol],'--k'),text(1e3,tol*2,'Tolérance'), 
		legend('erreur absolue','erreur relative',''), hold off
        set(gcf,'position',[281 47.4000 975.2000 724]), 
